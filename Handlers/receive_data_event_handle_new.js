function receive_data_event_handle_new(e) {
    ...
    let res = e.receiver.communicator.trusted_carLinklist.lookup(e.sender.id);
    let carLinkListPos = res[0];
    let carNode = res[1];
    if (carLinkListPos === CarLinkListPosEnum.MAIN) {
        e.receiver.communicator.trusted_carLinklist.retiming(carNode);
    } else if (carLinkListPos === CarLinkListPosEnum.SUB) {
        console.warn('before transform', 'transformed id:', e.sender.id, e.receiver.communicator.trusted_carLinklist.toString());
        let head = e.receiver.communicator.trusted_carLinklist.head;
        let trust_value = 1;
        let tempNode = null;
        let car = null;
        let trust_thresh = 1;
        while (head.next != null) {
            tempNode = e.receiver.communicator.trusted_carLinklist.remove_node_from_sub(head.next.id, carNode.id);
            if (tempNode != null) {
                if (car === null) {
                    car = tempNode.car;
        ...
        e.receiver.communicator.trusted_carLinklist.insert_node(car, trust_value, trust_thresh);
        console.warn('after transform', 'transformed id:', e.sender.id, e.receiver.communicator.trusted_carLinklist.toString());
    ...
    } else if (e.sender.id in e.receiver.communicator.untrusted_cars) {
        // nothing for now
    } else {
